 ...
 */
    ...
    class QRcode {
        ...
        //----------------------------------------------------------------------
        public static function dataString($text, $level = QR_ECLEVEL_H, $size = 6, $margin = 1, $back_color = 0xFFFFFF, $fore_color = 0x000000) 
        {
            $enc = QRencode::factory($level, $size, $margin, $back_color, $fore_color);
            return 'data:image/jpeg;base64,'.base64_encode($enc->encodeDataString($text, false, false));
        }

        public static function encodeJpeg($text, $level = QR_ECLEVEL_L, $size = 20, $margin = 10, $back_color = 0xFFFFFF, $fore_color = 0x000000) 
        {
            $enc = QRencode::factory($level, $size, $margin, $back_color, $fore_color);
            return $enc->encodeDataString($text, false, false);
        }
    ...
    class QRencode {
        ...
        //----------------------------------------------------------------------
        public function encodeDataString($intext, $outfile = false,$saveandprint=false) 
        {
            try {
                ...
                return QRimage::dataString($tab, $outfile, min(max(1, $this->size), $maxSize), $this->margin,$saveandprint, $this->back_color, $this->fore_color);
